...
const IncidentCard: React.FC<Props> = ({incident, wcif}) => {
    ...
    return (
        <Card backgroundColor="teal.500" color="white" textAlign="center" width="fit-content" cursor="pointer" onClick={() => navigate(`/incidents/${incident.id}`)}>
            <CardHeader>
                <Heading size="md">
                    {incident.result.person.name} ({incident.result.person.registrantId})
                </Heading>
            </CardHeader>
            <CardBody>
