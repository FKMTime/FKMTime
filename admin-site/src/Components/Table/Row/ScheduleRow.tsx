...
const ScheduleRow: React.FC<ScheduleRowProps> = ({ activity, events }): JSX.Element => {
    const eventId = activity.activityCode.split("-")[0];
    const event = events.find((event: Event) => event.id === eventId);
    const round = event?.rounds.find((round: Round) => round.id === activity.activityCode);
    ...
    return (
        <Tr key={activity.id}>
            ...
            <Td>{activity.childActivities.length === 0 ? "" : activity.childActivities.length}</Td>
        </Tr>
    );
}
