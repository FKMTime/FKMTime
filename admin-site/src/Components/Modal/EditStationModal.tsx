...
const EditStationModal: React.FC<EditStationModalProps> = ({ isOpen, onClose, station }): JSX.Element => {
        ...
        <Modal isOpen={isOpen} onClose={onClose} title="Create station">
            <Box display="flex" flexDirection="column" gap="5" as="form" onSubmit={handleSubmit}>
                <FormControl isRequired>
                    ...
                    <Input placeholder='Name' _placeholder={{ color: "white" }} disabled={isLoading} value={editedStation.name} onChange={(e) => setEditedStation({ ...editedStation, name: e.target.value })} />
                </FormControl>
                <FormControl isRequired>
                    <FormLabel>ESP ID</FormLabel>
                    <Input placeholder='ESP ID' type="text" _placeholder={{ color: "white" }} disabled={isLoading} value={editedStation.espId} onChange={(e) => setEditedStation({ ...editedStation, espId: e.target.value })} />
                </FormControl>
