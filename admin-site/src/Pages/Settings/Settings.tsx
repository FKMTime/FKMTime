...
const Settings = (): JSX.Element => {
    ...
    return (
        <>
            ...
            <Box display="flex" flexDirection="column" gap={3} as="form" width="20%" mt={5} onSubmit={handleSubmit}>
                <FormControl isRequired>
                    <FormLabel>Username</FormLabel>
                    <Input placeholder='Username' _placeholder={{ color: "white" }} value={settings.username} disabled={isLoading} onChange={(e) => setSettings({ ...settings, username: e.target.value })} />
                </FormControl>
                <FormControl isRequired>
                    <FormLabel>Email</FormLabel>
                    <Input placeholder='Email' _placeholder={{ color: "white" }} value={settings.email} disabled={isLoading} onChange={(e) => setSettings({ ...settings, email: e.target.value })} />
                </FormControl>
                <Button colorScheme="green" isLoading={isLoading} type="submit">Save</Button>
                <Button colorScheme="yellow" onClick={() => setIsOpenChangePasswordModal(true)}>Change password</Button>
            </Box>
